...
function TapeOverlay() {
  ...
  return (
    <AbsoluteFill style={{ opacity: intro, pointerEvents: 'none' }}>
      <div style={{ position: 'absolute', left: 70, top: 56, fontFamily: mono, fontSize: 28, letterSpacing: '0.14em', color: 'rgba(255,255,255,0.7)', ...chroma }}>
        HAFTA 38 · MAÇ KASETİ
      </div>
      <div style={{ position: 'absolute', right: 70, top: 56, fontFamily: mono, fontSize: 28, letterSpacing: '0.1em', color, textAlign: 'right', ...chroma }}>
        <span style={{ opacity: mode === 'pause' && !blink ? 0.25 : 1 }}>{label}</span>
        <div style={{ fontSize: 24, color: 'rgba(255,255,255,0.55)', marginTop: 10 }}>{tc(tapeTime(t), fps)}</div>
      </div>
      {/* Kayıt ışığı */}
      <div style={{ position: 'absolute', left: 70, top: 108, display: 'flex', alignItems: 'center', gap: 12, fontFamily: mono, fontSize: 22, color: 'rgba(255,255,255,0.45)' }}>
      ...
      <div style={{ position: 'absolute', left: 70, right: 70, bottom: 54, height: 4, background: 'rgba(255,255,255,0.12)', borderRadius: 2 }}>
        ...
        <div style={{ position: 'absolute', left: `${progress * 100}%`, top: -7, width: 18, height: 18, borderRadius: 9, background: C.text, transform: 'translateX(-9px)' }} />
      </div>
    </AbsoluteFill>
  );
}
...
function Scribble({ text, x, y, at, size = 44, rot = -4 }: { text: string; x: number; y: number; at: number; size?: number; rot?: number }) {
  ...
  return (
    <div style={{
      position: 'absolute', left: x, top: y, fontFamily: serif, fontStyle: 'italic', fontSize: size, color: C.gold,
...
function Subtitle({ text, from, to }: { text: string; from: number; to: number }) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        fontFamily: serif, fontStyle: 'italic', fontSize: 54, color: C.text, background: 'rgba(0,0,0,0.55)',
...
function ClipTradeDetail({ c }: { c: typeof MAC.clips[number] }) {
  ...
  const cw = 860, ch = 500;
  return (
    <div style={{ position: 'absolute', left: 150, top: 190, display: 'flex', gap: 50 }}>
      <Card style={{ padding: 22, borderRadius: 24 }}>
        <div style={{ fontFamily: mono, fontSize: 20, color: C.faint, letterSpacing: '0.14em', margin: '4px 6px 14px' }}>İŞLEM SONRASI GRAFİK</div>
        <div style={{ borderRadius: 14, overflow: 'hidden', background: 'rgba(0,0,0,0.3)' }}>
      ...
      <div style={{ width: 520, paddingTop: 12, fontFamily: sans }}>
        ...
        <div style={{ fontSize: 26, color: C.faint, marginTop: 8 }}>17 Eylül · 14:02</div>
        <div style={{ display: 'flex', gap: 60, marginTop: 40 }}>
          <div>
            <div style={{ fontSize: 20, letterSpacing: '0.14em', color: C.faint }}>RİSK</div>
            <div style={{ fontFamily: mono, fontSize: 38, color: C.text, marginTop: 8 }}>$202.50</div>
          </div>
          <div>
            <div style={{ fontSize: 20, letterSpacing: '0.14em', color: C.faint }}>SONUÇ</div>
            <div style={{ fontFamily: mono, fontSize: 38, color: C.red, marginTop: 8 }}>−$351.00</div>
          </div>
        </div>
        <div style={{ fontFamily: mono, fontSize: 120, color: C.red, letterSpacing: '-0.04em', marginTop: 26, lineHeight: 1 }}>−1.7R</div>
        <div style={{ fontSize: 20, letterSpacing: '0.14em', color: C.faint, marginTop: 40 }}>NOTUN</div>
        <div style={{ fontFamily: serif, fontStyle: 'italic', fontSize: 34, color: 'rgba(255,255,255,0.8)', marginTop: 10, lineHeight: 1.3 }}>
          “Stopu biraz genişlettim, dönecekti.”
        </div>
        <div style={{ display: 'flex', gap: 12, marginTop: 22 }}>
          {['Aşırı özgüvenli', 'Sabırsız'].map(e => (
...
function ListRow({ r, i, start }: { r: typeof LIST[number]; i: number; start: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 34, padding: '22px 26px', borderRadius: 16, opacity: p * (i === 0 ? 0.5 : 1), transform: `translateY(${(1 - p) * 14}px)`, fontFamily: sans }}>
      ...
      <span style={{ fontFamily: mono, fontSize: 30, color: 'rgba(255,255,255,0.45)', width: 120, textAlign: 'right' }}>{R}</span>
      <span style={{ marginLeft: 'auto', fontFamily: mono, fontSize: 36, fontWeight: 500, color: pnl >= 0 ? C.green : C.red }}>{signed(pnl)}.00</span>
    </div>
  );
}
...
function FlagMini({ title, desc, count, pnl, at, left, top }: { title: string; desc: string; count: number; pnl: number; at: number; left: number; top: number }) {
  ...
  return (
    <div style={{ position: 'absolute', left, top, opacity: p, transform: `translateX(${(1 - p) * 40}px)` }}>
      <Card style={{ width: 560, padding: '30px 34px' }}>
        <div style={{ fontSize: 20, letterSpacing: '0.18em', color: C.faint, fontWeight: 500 }}>DİSİPLİN · İŞARETLENEN</div>
        <div style={{ display: 'flex', gap: 24, marginTop: 22, alignItems: 'flex-start' }}>
          <div style={{ fontFamily: mono, fontSize: 64, lineHeight: 1, color: C.amber, width: 84, textAlign: 'right' }}>{count}</div>
          <div>
        ...
        <div style={{ fontFamily: mono, fontSize: 56, color: C.red, textAlign: 'right', letterSpacing: '-0.03em' }}>{signed(pnl)}</div>
      </Card>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'absolute', left: 150, top: 180 }}>
      <Card style={{ width: 1000, padding: '34px 30px' }}>
        <div style={{ fontFamily: serif, fontSize: 44, color: C.text, padding: '0 26px 10px' }}>Perşembe, 17 Eylül</div>
      ...
      <div key={i} style={{ width: cellW, height: cellH, borderRadius: 16, background: inMonth ? bg : 'transparent', padding: 12, display: 'flex', flexDirection: 'column', opacity: p, fontFamily: sans }}>
        ...
        {st && (
          <div style={{ marginTop: 'auto' }}>
            <div style={{ fontFamily: mono, fontSize: 22, fontWeight: 600, color: col }}>{signed(st[0])}</div>
            <div style={{ fontSize: 18, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>{st[1]} işlem</div>
          </div>
        )}
      </div>,
    );
  ...
  return (
    <div style={{ position: 'absolute', left: 150, top: 140 }}>
      <Card style={{ padding: '30px 34px' }}>
        <div style={{ fontFamily: serif, fontSize: 44, color: C.text, marginBottom: 18 }}>Eylül 2026</div>
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(7, ${cellW}px)`, gap }}>
          {heads.map(h => <div key={h} style={{ fontSize: 22, color: C.faint, fontFamily: sans, paddingLeft: 12 }}>{h}</div>)}
          {cells}
        </div>
      </Card>
    </div>
  ...
  return (
    <Card style={{ width: 1100, padding: '26px 30px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 34, fontFamily: sans, padding: '6px 10px' }}>
        ...
        <span style={{ fontFamily: mono, fontSize: 32, color: 'rgba(255,255,255,0.45)', opacity: filled }}>+1.8R</span>
        <span style={{ marginLeft: 'auto', position: 'relative', height: 50, display: 'flex', alignItems: 'center' }}>
          <span style={{ opacity: 1 - filled, fontSize: 26, fontWeight: 500, color: C.amber, background: 'rgba(251,191,36,0.12)', borderRadius: 999, padding: '9px 22px' }}>Tamamlanmadı</span>
          <span style={{ position: 'absolute', right: 0, opacity: filled, fontFamily: mono, fontSize: 40, fontWeight: 500, color: C.green }}>+$364.50</span>
        </span>
      </div>
    </Card>
  );
      ...
      {rows.map(([n, title, pnl], i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 30, padding: '20px 0', borderTop: i ? `1px solid ${C.line}` : 'none', opacity: p, transform: `translateX(${(1 - p) * 30}px)`, fontFamily: sans }}>
            <span style={{ fontFamily: mono, fontSize: 44, color: C.amber, width: 70, textAlign: 'right' }}>{n}</span>
            <span style={{ fontSize: 38, fontWeight: 500, color: C.text }}>{title}</span>
            <span style={{ marginLeft: 'auto', fontFamily: mono, fontSize: 40, color: C.red }}>{signed(pnl)}</span>
          </div>
        );
      })}
    </Card>
...
function M3({ at }: { at: number }) {
  const cells: [string, number, number, 'target' | 'loss', string][] = [
    ...
    ['BUGÜNKÜ LİMİTE KALAN', 500, 410, 'loss', 'bugün −$90'],
    ['TOPLAM KAYBA KALAN', 1000, 870, 'loss', 'taban $9,000'],
  ];
      ...
      {cells.map(([label, limit, left, kind, note]) => {
        ...
        return (
          <div key={label} style={{ flex: 1, fontFamily: sans }}>
            <div style={{ fontSize: 22, letterSpacing: '0.14em', color: C.faint, fontWeight: 500 }}>{label}</div>
            <div style={{ fontFamily: mono, fontSize: 62, color, marginTop: 14, letterSpacing: '-0.03em' }}>{money(now)}</div>
            ...
            <div style={{ fontFamily: mono, fontSize: 22, color: C.faint, marginTop: 12 }}>{note}</div>
          </div>
        );
      })}
    </Card>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: 24, color: 'rgba(255,255,255,0.4)' }}>şimdi</span>
          </div>
          <div style={{ fontSize: 36, fontWeight: 600, color: C.text, marginTop: 6 }}>Önemli haber yaklaşıyor</div>
          <div style={{ fontSize: 30, color: 'rgba(255,255,255,0.75)', marginTop: 4 }}>
            <span style={{ color: C.red }}>●</span> Non-Farm Employment Change · 60 dakika sonra
          </div>
        </div>
      </div>
    </div>
...
function M5({ at }: { at: number }) {
  ...
  const Frame = ({ label, candles, p, lo, hi }: { label: string; candles: ReturnType<typeof candlesFrom>; p: number; lo: number; hi: number }) => (
    <div style={{ opacity: p, transform: `translateY(${(1 - p) * 20}px)` }}>
      <div style={{ fontFamily: mono, fontSize: 20, color: C.faint, letterSpacing: '0.14em', marginBottom: 10 }}>{label}</div>
      <div style={{ borderRadius: 16, overflow: 'hidden', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 34 }}>
      <div style={{ display: 'flex', gap: 40 }}>
        <Frame label="İŞLEM ÖNCESİ" candles={PHOTO_A} p={p1} lo={1.2680} hi={1.2740} />
        <Frame label="İŞLEM SONRASI" candles={PHOTO_B} p={p2} lo={1.2685} hi={1.2775} />
      </div>
      <div style={{ opacity: pn, fontFamily: serif, fontStyle: 'italic', fontSize: 38, color: 'rgba(255,255,255,0.8)' }}>
        “Londra açılışını bekledim. Plana sadık kaldım.”
      </div>
    </div>
  );
}
...
function M6({ at }: { at: number }) {
  const week: [string, number, number][] = [['Pzt', 240, 2], ['Sal', 180, 2], ['Çar', -60, 1], ['Per', 310, 2], ['Cum', 150, 1]];
  return (
      ...
      {week.map(([d, pnl, n], i) => {
        ...
        return (
          <div key={d} style={{
            ...
            <span style={{ marginTop: 'auto', fontFamily: mono, fontSize: 44, fontWeight: 600, color: col }}>{signed(pnl)}</span>
            <span style={{ fontSize: 22, color: 'rgba(255,255,255,0.4)', marginTop: 4 }}>{n} işlem</span>
          </div>
        );
      })}
    </div>
  ...
  const Bar = ({ label, pnl, n, color, max }: { label: string; pnl: number; n: number; color: string; max: number }) => (
    <div style={{ fontFamily: sans }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        ...
        <span style={{ fontSize: 24, color: C.faint }}>{n} işlem</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 30, marginTop: 18 }}>
        ...
        <span style={{ fontFamily: mono, fontSize: 58, color, width: 280, textAlign: 'right', letterSpacing: '-0.03em' }}>{signed(pnl * p)}</span>
      </div>
    </div>
  );
  return (
...
export function MacKod({ music = true }: { music?: boolean }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const t = frame / fps;
  ...
  return (
    <AbsoluteFill style={{ background: C.bg }}>
        ...
        <CrtOff>
          <Footage>
            ...
            <Window from={K1.start} to={K1.end}>
              ...
              <DrawArrow d="M 700 430 C 684 400, 690 368, 702 338" head="M 680 360 L 703 334 L 724 360" at={K1.draw + 0.45} />
              <Scribble text="planladığının 1,7 katı" x={1492} y={458} at={K1.data} size={38} />
            </Window>
            <Window from={K2.start} to={K2.end}>
              <ClipList c={K2} />
              <DrawCircle cx={256} cy={452} rx={82} ry={112} at={K2.draw} rot={0} />
              <DrawArrow d="M 158 404 C 92 414, 92 488, 158 500" head="M 134 482 L 160 500 L 134 518" at={K2.draw + 0.4} />
              <Scribble text="4 dk" x={24} y={420} at={K2.draw + 0.75} size={46} rot={-8} />
              <FlagMini at={K2.data} left={1200} top={260} title="Hemen geri girme" desc="Kaybettikten sonra 15 dakika içinde açılan işlemler" count={14} pnl={-1380} />
            </Window>
            {/* Son klip, televizyon kapanırken de ekranda kalmalı: kapanış onu sıkıştırıyor. */}
            <Window from={K3.start} to={MAC.stop + 0.8}>
              <ClipCalendar c={K3} />
              <DrawCircle cx={704} cy={571} rx={96} ry={72} at={K3.draw} />
              <Scribble text="← 11 işlem!" x={946} y={545} at={K3.draw + 0.5} size={46} rot={-5} />
              <FlagMini at={K3.data} left={1316} top={330} title="Aşırı işlem" desc="Olağan gününün iki katından çok işlem açtığın günler" count={29} pnl={-940} />
            </Window>
          </Footage>
          <Snow />
          <TapeTexture />
      ...
      {/* ── Montaj ── */}
      {[
        ['Her işlem kendiliğinden kaydolur.', M1, 1.4],
        ['Her alışkanlık işaretlenir.', M2, 1.2],
        ['Sınırın her an gözünün önünde.', M3, 1.08],
        ['Kırmızı haberden önce haber verir.', M4, 1.3],
        ['Grafiğin ve notun, hepsi yerinde.', M5, 1.2],
