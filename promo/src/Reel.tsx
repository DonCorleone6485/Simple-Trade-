...
function Label({ n, text, at }: { n: string; text: string; at: number }) {
  ...
  return (
    <div style={{ position: 'absolute', left: 90, top: 80, display: 'flex', alignItems: 'center', gap: 22, fontFamily: sans }}>
      ...
      <span style={{ overflow: 'hidden', display: 'inline-block' }}>
        <span style={{ display: 'inline-block', fontSize: 28, fontWeight: 600, letterSpacing: '0.24em', color: C.text, transform: `translateY(${(1 - w) * 110}%)` }}>{text}</span>
      </span>
    </div>
  );
}
...
function Kinetic() {
  ...
  const word = ['KAYDET', 'ANALİZ ET', 'DÜZELT', 'TEKRARLA'][which];
  ...
  return (
    <AbsoluteFill style={{ background: bgs[which], alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontFamily: sans, fontWeight: 800, fontSize: 250, letterSpacing: '-0.05em', color: fgs[which], transform: `scale(${pulse(t, 0.025)}) translateY(${Math.sin(k * Math.PI) * -6}px)` }}>
        {content}
      </div>
...
function AutoSync() {
  ...
  return (
    <AbsoluteFill style={{ background: C.bg }}>
      <Label n="01" text="OTOMATİK KAYIT" at={s} />
      ...
      <div style={{ position: 'absolute', left: 90, bottom: 80, fontFamily: sans, fontSize: 34, color: C.dim, opacity: pr(t, s + b(4.5), s + b(5.5)) }}>
        MetaTrader'dan <span style={{ color: C.gold }}>kendiliğinden.</span>
      </div>
    </AbsoluteFill>
  );
}
...
function Discipline() {
  ...
  const flags: [number, string, number][] = [[14, 'Hemen geri girme', -1380], [6, 'Riski büyütme', -2240], [29, 'Aşırı işlem', -940]];
  ...
  return (
    <AbsoluteFill style={{ background: C.bg }}>
      <Label n="02" text="DİSİPLİN ANALİZİ" at={s} />
      ...
      <AbsoluteFill style={{ background: `rgba(120,30,30,${0.25 * inside})`, opacity: inside, alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 36 }}>
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {flags.map(([n, title, pnl], i) => {
            ...
            return (
              <div key={i} style={{ fontFamily: sans, padding: '22px 30px', borderRadius: 18, background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(255,255,255,0.1)', transform: `translateY(${(1 - p) * 60}px)`, opacity: Math.min(1, p * 2), display: 'flex', gap: 16, alignItems: 'baseline' }}>
...
function Prop() {
  ...
  const items: [string, number, number, string][] = [
    ['HEDEFE KALAN', 1000, 640, C.gold], ['BUGÜNKÜ LİMİTE KALAN', 500, 188, C.amber], ['TOPLAM KAYBA KALAN', 1000, 780, C.green],
  ];
  return (
    <AbsoluteFill style={{ background: C.bg }}>
      <Label n="03" text="PROP SAYACI" at={s} />
            ...
            <React.Fragment key={i}>
              <div style={{
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                  ...
                  <div style={{ fontFamily: sans, fontSize: 17, letterSpacing: '0.14em', color: C.faint, marginTop: 8 }}>{label}</div>
                </div>
              </div>
              {/* Uygulamadaki hâli */}
              <div style={{ position: 'absolute', left: 360, width: 1200, top: barY, opacity: morph, transform: `translateX(${(1 - morph) * 80}px)`, fontFamily: sans }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <span style={{ fontSize: 24, letterSpacing: '0.14em', color: C.faint }}>{label}</span>
...
function Sessions() {
  ...
  const sess: [string, number, number, string][] = [
    ['SYDNEY', 21, 30, '#60a5fa'], ['TOKYO', 0, 9, C.lilac], ['LONDRA', 7, 16, C.gold], ['NEW YORK', 12, 21, C.green],
  ];
  ...
  return (
    <AbsoluteFill style={{ background: C.bg }}>
      <Label n="04" text="SEANS & HABER" at={s} />
      {/* 24 saatlik cetvel */}
      ...
      {sess.map(([name, a, z, col], i) => {
        ...
        return segs.map(([sa, sz], k) => (
          <div key={`${i}-${k}`} style={{
            ...
            display: 'flex', alignItems: 'center', paddingLeft: 20, fontFamily: sans, fontWeight: 700, fontSize: 24, letterSpacing: '0.14em', color: col, overflow: 'hidden', whiteSpace: 'nowrap',
          }}>{k === 0 ? name : ''}</div>
        ));
      })}
      {/* Oynatma çizgisi */}
      ...
      <div style={{ position: 'absolute', right: 110, top: 900 + (1 - toast) * 260, width: 620, opacity: Math.min(1, toast * 2) }}>
        <div style={{ display: 'flex', gap: 20, alignItems: 'center', padding: '22px 26px', borderRadius: 24, background: 'rgba(38,38,44,0.92)', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 30px 80px rgba(0,0,0,0.6)', fontFamily: sans }}>
          ...
          <div>
            <div style={{ fontSize: 28, fontWeight: 600, color: C.text }}>Önemli haber yaklaşıyor</div>
            <div style={{ fontSize: 24, color: C.dim, marginTop: 4 }}><span style={{ color: C.red }}>●</span> NFP · 60 dakika sonra</div>
          </div>
        </div>
      </div>
    </AbsoluteFill>
...
function Tile({ i, t }: { i: number; t: number }) {
  ...
  const inner: React.ReactNode = (() => {
    switch (kind) {
      ...
      case 'word': return <div style={{ fontFamily: sans, fontWeight: 800, fontSize: 88, letterSpacing: '-0.05em', color: C.text }}>{['KAYDET', 'ANALİZ', 'DÜZELT'][Math.floor((t * 2 + i) % 3)]}</div>;
...
function Wall() {
  ...
  const faces = ['KAYDET', 'ANALİZ ET', 'DÜZELT', 'SINIR', 'STJ', 'HABER', 'SEANS', 'DİSİPLİN', 'TEKRARLA'];
  return (
      ...
      {t >= s + b(1) && (
        <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 34, transform: `scale(${punch})`, opacity: 1 - out }}>
          ...
          <div style={{ display: 'flex', gap: 40, alignItems: 'baseline', opacity: url, transform: `translateY(${(1 - url) * 20}px)` }}>
            <span style={{ fontFamily: serif, fontSize: 46, color: C.dim }}>Disiplin, basitlikle.</span>
...
export function Reel({ music = true }: { music?: boolean }) {
  const t = useT();
  const f = useCurrentFrame();
