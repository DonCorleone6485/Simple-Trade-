...
import { C, mono, faSans as sans, faSerif as serif, signed } from './theme';
import {
...
const PAIRS = [
  {
    ...
    kind: 'flag' as const,
    title: 'بازگشت فوری',
    desc: 'معاملات باز شده تا ۱۵ دقیقه پس از ضرر',
    count: 14, pnl: -1380,
  },
  {
    claim: '«ریسکم را\nهرگز بالا نمی‌برم.»',
    kind: 'flag' as const,
    title: 'افزایش ریسک',
    desc: 'افزایش ریسک بیش از ۱.۵ برابر پس از ضرر',
    count: 6, pnl: -2240,
  },
  {
    claim: '«جمعه‌ها\nروزِ من است.»',
...
function ClaimBlock({ text, s0 }: { text: string; s0: number }) {
  ...
  return (
    <div style={{ position: 'relative', opacity: dimmed }}>
      <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={text} start={s0 + P.claim} size={86} stagger={0.09} />
...
function FlagCard({ s0, title, desc, count, pnl }: { s0: number; title: string; desc: string; count: number; pnl: number }) {
  ...
  return (
    <Card style={{ fontFamily: sans, width: 920 }}>
      <div style={{ fontSize: 24, letterSpacing: 0, color: C.faint, fontWeight: 500, marginBottom: 34 }}>
        عادت‌های علامت‌خورده
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <div style={{ fontSize: 28, color: C.faint }}>نتیجه‌ی این معاملات</div>
        <div style={{
...
function HeatCard({ s0, pnl }: { s0: number; pnl: number }) {
  ...
  return (
    <Card style={{ fontFamily: sans, width: 920, padding: '36px 40px' }}>
      <div style={{ fontSize: 24, letterSpacing: 0, color: C.faint, fontWeight: 500, marginBottom: 26 }}>نقشه حرارتی</div>
      <div style={{ display: 'grid', gridTemplateColumns: '118px repeat(6, 1fr)', gap: 8, alignItems: 'center' }}>
        <div />
        {HOURS.map(h => (
        ...
        {HEAT.map((row, di) => (
          <React.Fragment key={di}>
            <div style={{ fontSize: 22, color: di === 4 ? C.text : 'rgba(255,255,255,0.5)', fontWeight: 500, opacity: di === 4 ? 1 : 1 - 0.5 * hot }}>{DAYS[di]}</div>
            {row.map(([n, dir], hi) => {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', opacity: callout, transform: `translateY(${(1 - callout) * 12}px)` }}>
        <div style={{ fontSize: 30, color: C.text }}>جمعه <span style={{ color: C.faint, fontFamily: mono, fontSize: 26, direction: 'ltr', unicodeBidi: 'isolate' }}>12:00–16:00</span></div>
  ...
  return (
    <Exit at={s0 + P.exit}>
      <AbsoluteFill style={{ alignItems: 'center', paddingTop: 420 }}>
        <Kicker family={sans} spacing="0" text="تو" start={s0 + 0.05} />
        <div style={{ height: 34 }} />
        <ClaimBlock text={d.claim} s0={s0} />
        <div style={{ height: 120 }} />
        <Kicker family={sans} spacing="0" text="ژورنالِ تو" start={s0 + P.answer - 0.15} color={C.lilac} />
        <div style={{ height: 34 }} />
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        ...
        <div style={{ fontSize: 26, color: C.faint }}>{n} معامله</div>
      </div>
...
function CompareScene() {
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', paddingTop: 400 }}>
      <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={'همان تریدر.\nهمان استراتژی.'} start={s + 0.1} size={92} stagger={0.1} />
      <div style={{ height: 110 }} />
      <Rise start={s + 0.9}>
        <Card style={{ fontFamily: sans, width: 920, display: 'flex', flexDirection: 'column', gap: 54 }}>
          <Bar label="معاملات مطابق قوانین" pnl={2140} n={38} max={4860} start={AYNA.compareBars} color={C.green} />
          <Bar label="معاملات علامت‌خورده" pnl={-4860} n={26} max={4860} start={AYNA.compareBars + 0.35} color={C.red} />
        </Card>
      </Rise>
      <div style={{ height: 90 }} />
      <div style={{ opacity: tail, transform: `translateY(${(1 - tail) * 14}px)`, fontFamily: serif, fontSize: 56, color: C.dim, fontStyle: 'italic' }}>
        فرق در استراتژی نیست.
      </div>
    </AbsoluteFill>
  );
}
  ...
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', transform: `translate(${shake}px, ${shake * 0.4}px)` }}>
      <div style={{ opacity: line1Dim }}>
        <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={'می‌توانی به خودت\nدروغ بگویی.'} start={AYNA.line1} size={104} stagger={0.13} dur={0.7} />
      </div>
      <div style={{ height: 70 }} />
      <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={'به ژورنالت\nنمی‌توانی.'} start={AYNA.line2 - 0.04} size={104} stagger={0.05} dur={0.35}
        accent={['نمی‌توانی']} />
...
export function AynaFa({ music = true }: { music?: boolean }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <AbsoluteFill style={{ background: C.bg, direction: 'rtl' }}>
      ...
      <Window from={AYNA.end} to={AYNA.duration}>
        <Endcard start={AYNA.end} line="خودت را همان‌طور که هستی ببین." cta="رایگان شروع کنید" lineFamily={serif} ctaFamily={sans} stacked logoWidth={520} gap={80} />
      </Window>
