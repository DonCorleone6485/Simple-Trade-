...
import { C, mono, faSans as sans, faSerif as serif } from './theme';
...
function Terminal() {
  ...
  return (
    <Card style={{ fontFamily: sans, width: 880, padding: '30px 40px 34px', position: 'relative', direction: 'ltr' }}>
      ...
      <div style={{ fontSize: 22, color: C.faint, letterSpacing: 0, marginTop: 6 }}>METATRADER 5</div>
...
function Row({ time, symbol, side, r, amount, pill, highlight = 0, dim = 1 }: {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: mono, fontSize: 26, color: 'rgba(255,255,255,0.45)', width: 110, textAlign: 'left', direction: 'ltr' }}>{r ?? ''}</span>
      <span style={{ marginInlineStart: 'auto', fontFamily: mono, fontSize: 32, fontWeight: 500, color: win ? C.green : C.red }}>
        {pill ? (
        ...
        ) : <span style={{ direction: 'ltr', display: 'inline-block' }}>{amount}</span>}
      </span>
    </div>
  );
}
...
function Journal() {
  ...
  const rowH = 86;
  return (
    <Card style={{ fontFamily: sans, width: 880, padding: '30px 26px 22px' }}>
      ...
      <div style={{ fontSize: 24, color: 'rgba(255,255,255,0.55)', padding: '6px 18px 8px' }}>امروز</div>
...
function Caption({ text, from, to, accent }: { text: string; from: number; to: number; accent?: string[] }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 222, left: 0, right: 0, opacity: 1 - out, filter: `blur(${out * 8}px)` }}>
      <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={text} start={from} size={70} stagger={0.08} accent={accent} />
    </div>
  );
}
...
export function HicFa({ music = true }: { music?: boolean }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const t = frame / fps;
  ...
  return (
    <AbsoluteFill style={{ background: C.bg, direction: 'rtl' }}>
      ...
      <Window from={0} to={HIC.end}>
        <Exit at={HIC.end - 0.3} dur={0.3}>
          ...
          <div style={{ opacity: 1 - back }}>
            <Caption text="تو وارد معامله شو." from={HIC.cap1} to={HIC.cap2 - 0.05} />
            <Caption text="ما ثبتش می‌کنیم." from={HIC.cap2} to={HIC.cap3 - 0.05} accent={['ثبتش']} />
            <Caption text="بسته که شد، ما کاملش می‌کنیم." from={HIC.cap3} to={HIC.verdict} accent={['کاملش']} />
          </div>

          {t >= HIC.verdict - 0.1 && (
            <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center' }}>
              <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={'حتی یک دکمه هم\nدر ژورنال نزدی.'} start={HIC.verdict} size={96} stagger={0.11} dur={0.6} accent={['نزدی']} />
            </AbsoluteFill>
          )}
        </Exit>
      </Window>
      ...
      <Window from={HIC.end} to={HIC.duration}>
        <Endcard start={HIC.end} line="خودکار با متاتریدر ۵." cta="رایگان شروع کنید" lineFamily={serif} ctaFamily={sans} stacked logoWidth={520} gap={80} />
      </Window>
