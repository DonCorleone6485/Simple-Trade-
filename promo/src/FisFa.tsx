// ÜRETİLMİŞ DOSYA — scripts/make_fa.py. Kaynağı düzenleyip betiği çalıştır.
...
import { C, faSerif as serif, mono, faSans as sans, money } from './theme';
import {
...
const LINES: Line[] = [
  ...
  { kind: 'center', text: '★  چالش پراپ  ★', at: H, h: 58, bold: true, size: 36 },
  { kind: 'center', text: 'رسید شماره 0008', at: H + 0.12, h: 44, size: 26 },
  ...
  { kind: 'total', label: 'جمع کل', value: money(TOTAL), at: FIS.total, h: 92, big: true },
  { kind: 'total', label: 'پی‌اوت دریافتی', value: '$0', at: FIS.payout, h: 54 },
  { kind: 'rule', at: FIS.rule2, h: 34 },
  { kind: 'center', text: 'از خرید شما سپاسگزاریم', at: FIS.thanks, h: 48, size: 28 },
  { kind: 'center', text: 'باز هم منتظرتان هستیم :)', at: FIS.again, h: 60, size: 30, bold: true },
...
function LineView({ l }: { l: Line }) {
  const base: React.CSSProperties = { height: l.h, display: 'flex', alignItems: 'center', fontFamily: sans, color: INK };
  switch (l.kind) {
    case 'gap': return <div style={{ height: l.h }} />;
    case 'center':
      return <div style={{ ...base, justifyContent: 'center', fontSize: l.size ?? 28, fontWeight: l.bold ? 700 : 400, letterSpacing: 0 }}>{l.text}</div>;
    case 'rule':
      return <div style={{ ...base }}><div style={{ flex: 1, borderTop: `3px dashed ${INK}`, opacity: 0.55 }} /></div>;
    case 'cols':
      return (
        <div style={{ ...base, fontSize: 22, opacity: 0.6, letterSpacing: 0 }}>
          <span style={{ width: 118 }}>تاریخ</span><span style={{ flex: 1 }}>حساب</span>
          <span style={{ width: 120, textAlign: 'left' }}>هزینه</span><span style={{ width: 200, textAlign: 'left' }}>وضعیت</span>
        </div>
      );
    case 'row':
      return (
        <div style={{ ...base, fontSize: 29 }}>
          ...
          <span style={{ flex: 1 }}>چالش {l.acct}</span>
          <span style={{ width: 120, textAlign: 'left', direction: 'ltr' }}>${l.price}</span>
          <span style={{ width: 200, textAlign: 'left', fontWeight: 700 }}>✗ رد شد</span>
        </div>
      );
    case 'total':
      return (
...
function Printer() {
  ...
  return (
    <div style={{ position: 'absolute', left: 60, right: 60, top: SLOT_Y - 8, bottom: -40 }}>
      ...
      <div style={{ position: 'absolute', left: 70, top: 58, fontFamily: sans, fontSize: 22, letterSpacing: 0, color: 'rgba(255,255,255,0.18)' }}>THERMAL · 80MM</div>
    </div>
  );
}
...
function PropCell({ label, limit, left, note, kind }: { label: string; limit: number; left: number; note: string; kind: 'target' | 'loss' }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 24, letterSpacing: 0, color: C.faint, fontWeight: 500 }}>{label}</div>
...
function PropScene() {
  return (
    <AbsoluteFill style={{ alignItems: 'center', paddingTop: 360 }}>
      <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={'هر لحظه بدان\nتا مرزت چقدر مانده.'} start={FIS.prop} size={80} stagger={0.08} accent={['هر', 'لحظه']} />
      <div style={{ height: 90 }} />
      <Rise start={FIS.propCard}>
        <Card style={{ fontFamily: sans, width: 920, display: 'flex', flexDirection: 'column', gap: 50 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 34, fontWeight: 500, color: C.text }}>Instant 10K</div>
            <div style={{ fontSize: 22, letterSpacing: 0, color: C.lilac, border: '1px solid rgba(167,139,250,0.35)', borderRadius: 999, padding: '8px 18px' }}>PROP</div>
          </div>
          <PropCell kind="target" label="تا هدف" limit={1000} left={640} note="هدف ⁦$1,000⁩" />
          <PropCell kind="loss" label="تا حد امروز" limit={500} left={188} note="امروز ⁦−$312⁩" />
          <PropCell kind="loss" label="تا ضرر کل" limit={1000} left={780} note="کف ⁦$9,000⁩" />
        </Card>
      </Rise>
    </AbsoluteFill>
  );
...
export function FisFa({ music = true }: { music?: boolean }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const t = frame / fps;
  ...
  return (
    <AbsoluteFill style={{ background: C.bg, direction: 'rtl' }}>
      ...
      <Window from={FIS.line} to={FIS.prop}>
        <Exit at={FIS.prop - 0.35} dur={0.35}>
          <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center' }}>
            <Words family={serif} weight={600} letterSpacing="0" lineHeight={1.4} text={'همان اشتباه را\nدوباره نخر.'} start={FIS.line} size={104} stagger={0.2} dur={0.7} accent={['نخر']} />
          </AbsoluteFill>
        </Exit>
      </Window>
      ...
      <Window from={FIS.end} to={FIS.duration}>
        <Endcard start={FIS.end} line="ژورنالی برای حساب پراپ تو." cta="رایگان شروع کنید" lineFamily={serif} ctaFamily={sans} stacked logoWidth={520} gap={80} />
      </Window>
